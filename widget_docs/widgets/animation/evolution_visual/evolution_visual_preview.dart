...
import 'package:flutter_custom_widget_docs/widgets/animation/evolution_visual.dart';
import 'package:flutter_custom_widget_docs/widgets/animation/evolution_visual.doc_widget.dart';

import '../../preview/device_frame_switcher.dart';
import '../../preview/preview_screen.dart';
import '../../preview/demo_host.dart';

final _documentation = EvolutionVisualDocWidget();

final evolutionVisualPreview = ElementPreview(
  document: _documentation,
  previews: [..._previews],
);

final _previews = renderPreview(
  title: "Evolution Visual",
  description:
      "A hand-off animation: A shakes, shrinks away, a burst of rays covers the moment of the swap, then B pops in. The three phases deliberately overlap on one controller — the rays begin before the old thing has finished shrinking and the new one arrives before they fade — so it reads as a single motion instead of three clips spliced together. The burst exists to hide the cut; without it the eye catches the substitution.",
  dependencies: _documentation.dependencies,
  code: r'''
EvolutionVisual(from: '🥚', to: '🐣'),
  ''',
  widget: DeviceFrameSwitcher(
    child: DemoSurface(
      child: const EvolutionVisual(from: '🥚', to: '🐣', size: 120),
    ),
  ),
  widgetCode: r'''
