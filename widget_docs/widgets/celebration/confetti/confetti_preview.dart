...
import 'package:flutter_custom_widget_docs/widgets/celebration/confetti.doc_widget.dart';

import '../../preview/device_frame_switcher.dart';
import '../../preview/preview_screen.dart';
import '../../preview/demo_host.dart';

final _documentation = ConfettiDocWidget();

final ConfettiController _demoController = ConfettiController();

final confettiPreview = ElementPreview(
...
final _previews = renderPreview(
  title: "Confetti",
  description:
      "Hand-rolled confetti in place of the `confetti` package. Every flake carries its own position, velocity, spin and flip phase under gravity and drag, fading out at the end of its life — all on one `AnimationController` and one `CustomPainter`, because a widget per flake means 30 render objects jumping every frame. The particle set is seeded per shot, so a rebuild mid-flight does not teleport the flakes. Always wrapped in `IgnorePointer`: flakes cover the list underneath, and nobody tapping quickly should lose a tap to a piece of paper.",
  dependencies: _documentation.dependencies,
  code: r'''
final controller = ConfettiController();
...
Confetti(controller: controller, particleCount: 40),
...
controller.play();
  ''',
    ...
    child: DemoSurface(
      child: SizedBox(
        ...
        child: Stack(
          ...
          children: [
            Confetti(controller: _demoController, particleCount: 40),
            OutlinedButton(
              onPressed: _demoController.play,
              child: const Text('Fire confetti'),
            ),
          ],
        ),
      ),
