...
import 'package:flutter_custom_widget_docs/widgets/illustration/jar_visual.dart';
import 'package:flutter_custom_widget_docs/widgets/illustration/jar_visual.doc_widget.dart';

import '../../preview/device_frame_switcher.dart';
import '../../preview/preview_screen.dart';
import '../../preview/demo_host.dart';

final _documentation = JarVisualDocWidget();

final jarVisualPreview = ElementPreview(
...
final _previews = renderPreview(
  title: "Jar Visual",
  description:
      "A glass jar filling up as `fill` rises, painted so it accepts *any* colour and *any* level without needing an image file per combination. `JarContent` swaps what sits inside — coins, a plant, hearts, a book — so one shape covers several kinds of goal. The level is clamped to 0.15..0.85: a completely empty jar looks broken, and a full one leaves no room for the lid.",
  dependencies: _documentation.dependencies,
  code: r'''
JarVisual(accent: CustomColors.gold, fill: 0.6),
  ''',
  widget: DeviceFrameSwitcher(
    child: DemoSurface(
      child: Wrap(
        ...
        children: const [
          JarVisual(accent: CustomColors.gold, fill: 0.65, size: 96),
          JarVisual(
            accent: CustomColors.success,
            content: JarContent.plant,
            fill: 0.4,
            size: 96,
          ),
          JarVisual(
            accent: CustomColors.error,
            content: JarContent.hearts,
            fill: 0.85,
            size: 96,
          ),
          JarVisual(
            accent: CustomColors.info,
            content: JarContent.book,
            fill: 0.3,
            size: 96,
          ),
        ],
