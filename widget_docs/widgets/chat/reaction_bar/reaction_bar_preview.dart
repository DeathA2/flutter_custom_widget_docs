...
import 'package:flutter_custom_widget_docs/widgets/chat/reaction_bar.doc_widget.dart';

import '../../preview/device_frame_switcher.dart';
import '../../preview/preview_screen.dart';
import '../../preview/demo_host.dart';

final _documentation = ReactionBarDocWidget();

final GlobalKey _bubbleKey = GlobalKey();

final reactionBarPreview = ElementPreview(
...
final _previews = renderPreview(
  title: "Reaction Bar",
  description:
      "The emoji bar a long press raises over a message. `showReactionBar` anchors it to the bubble itself rather than centring it on screen: a picker that appears somewhere else makes the reader hunt for which message they are reacting to, and with several short bubbles on one screen that is a real mis-tap risk. The barrier stays faint so the conversation is still readable behind it. It returns the chosen emoji through `Navigator.pop`, or null if dismissed — which is why the bar below is only a still image; a live one would pop the preview frame's own route.",
...
final emoji = await showReactionBar(
  ...
  widget: DeviceFrameSwitcher(
    child: DemoValue<String?>(
      initial: null,
      builder:
          (BuildContext context, String? picked, ValueChanged<String?> set) =>
              DemoSurface(
                  ...
                  children: [
                    ...
                    GestureDetector(
                      ...
                      onLongPress: () async {
                        ...
                        final String? emoji = await showReactionBar(
                          context: context,
                          anchor: anchor,
                        );
                        set(emoji);
