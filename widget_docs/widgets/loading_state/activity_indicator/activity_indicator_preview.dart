...
import 'package:flutter_custom_widget_docs/widgets/loading_state/activity_indicator.doc_widget.dart';

import '../../preview/device_frame_switcher.dart';
import '../../preview/preview_screen.dart';
import '../../preview/demo_host.dart';

final _documentation = ActivityIndicatorDocWidget();

final activityIndicatorPreview = ElementPreview(
...
final _previews = renderPreview(
  ...
  description:
      "Cross-platform loading spinner: a hand-painted Cupertino wheel on iOS and macOS, Material everywhere else. One widget, so call sites never have to remember which OS they are on. `radius` and `color` make it fit inside a button as readily as inside a full-screen overlay.",
  ...
  widget: DeviceFrameSwitcher(
    child: DemoSurface(
      child: Row(
        ...
        children: [
          const ActivityIndicator(radius: 8),
          const ActivityIndicator(radius: 12),
          ActivityIndicator(radius: 18, color: CustomColors.brand),
        ],
      ),
    ),
  ),
