...
import 'package:flutter_custom_widget_docs/widgets/inputs/pin_code_input.dart';
import 'package:flutter_custom_widget_docs/widgets/inputs/pin_code_input.doc_widget.dart';

import '../../preview/device_frame_switcher.dart';
import '../../preview/preview_screen.dart';
import '../../preview/demo_host.dart';

final _documentation = PinCodeInputDocWidget();

final pinCodeInputPreview = ElementPreview(
...
final _previews = renderPreview(
  ...
  description:
      "One real `TextField` sits behind six painted boxes — not six fields. A row of six fields is the obvious shape and it breaks on Android: backspace in an already-empty box fires no `onChanged`, so focus can never step back and the user is stuck at box four unable to fix box three. With a single field, backspace is native OS behaviour, pasting a six-digit code fills every box at once, a selection cannot straddle two boxes, and SMS autofill has somewhere to land. A wrong code puts the boxes in an error state and shakes the row once.",
      ...
      builder: (BuildContext context, String value, ValueChanged<String> set) =>
          DemoSurface(
            child: PinCodeInput(
              ...
              length: 6,
              allowReveal: true,
              errorText: value.length == 6 && value != '123456'
